...
4. **Run remote tests and iterate until they pass**:
   ...
   - **Note**: The gh CLI uses its config file (`~/.config/gh/hosts.yml`) for auth.
     Cursor cloud agents authenticate gh via config file, not environment variables.
   - If permission denied (token lacks 'workflow' scope): `git push -u origin HEAD && make ci-watch`
...
1. Read the error output (printed directly to console by the remote test scripts)
2. Fix the issues
3. Commit and re-run `make test-host-remote` or `make test-guest-remote`
4. Repeat until tests pass

**Prefer `make test-*-remote` over `make ci-watch`**: The remote test scripts are the primary method — they trigger a fresh build, wait for it, and stream errors. Use `ci-watch` only as a fallback when remote scripts can't trigger workflows (e.g., missing workflow scope).

Do NOT ask the user to copy CI errors. The remote test commands show errors directly.
...
make test-guest-remote  # triggers Windows build, waits, streams errors

# Fallback: watch existing CI run (only if remote scripts fail due to permissions)
make ci-watch           # tails latest run for current branch, shows errors on failure
```

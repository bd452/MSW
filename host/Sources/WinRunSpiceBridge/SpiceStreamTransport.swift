...
// MARK: - macOS Implementation
            ...
            if let character {
                character.withCString { charPtr in
                    var cEvent = winrun_keyboard_event(
                        window_id: event.windowID,
                        event_type: winrun_key_event_type(
                            rawValue: UInt32(event.eventType.rawValue)),
        ...
        func sendDragDropEvent(_ event: DragDropEvent) {
            ...
            cFiles.withUnsafeBufferPointer { filesBuffer in
                var cEvent = winrun_drag_event(
                    ...
                    allowed_operations: winrun_drag_operation(
                        rawValue: UInt32(event.allowedOperations.first?.rawValue ?? 0)),
                    selected_operation: winrun_drag_operation(
                        rawValue: UInt32(event.selectedOperation?.rawValue ?? 0))
                )
    ...
    private let spiceMetadataThunk:
        @convention(c) (

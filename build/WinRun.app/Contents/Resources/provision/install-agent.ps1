...
function Find-AgentInstaller {
    ...
    # autounattend.xml copies from CD-ROM to provision directory during setup
    $searchPaths = @(
        "$Script:ProvisionDir\WinRunAgent.msi",  # Copied by autounattend.xml (primary)
        "C:\WinRun\WinRunAgent.msi",             # Pre-copied location
        "E:\WinRunAgent.msi",                    # Secondary CD-ROM (autounattend ISO)
        "F:\WinRunAgent.msi",                    # Fallback CD-ROM locations
        "G:\WinRunAgent.msi",
        "H:\WinRunAgent.msi",
        "D:\WinRunAgent.msi"                     # Windows ISO (unlikely)
    )

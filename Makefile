...
.PHONY: help bootstrap setup-wine-tests verify-wine-tests build build-host build-guest \
        run run-app run-prod package \
        test test-host test-guest test-guest-wine \
        ...
        check-linux check-linux-mid install-daemon uninstall-daemon check-deps qemu-bootstrap qemu-doctor \
...
help:
	...
	@echo "  brew-check     Verify Brewfile.lock matches installed versions"
...
# ============================================================================
# Brewfile defines what packages to install
# Brewfile.lock records installed versions (used for CI cache key)
#
# Workflow:
#   - Add/remove packages: edit Brewfile, then run 'make brew-sync'
#   - Upgrade packages: run 'make brew-sync'
#   - Commit both Brewfile and Brewfile.lock to update CI cache

# Extract package names from Brewfile (lines starting with 'brew')
BREW_PACKAGES := $(shell grep '^brew' $(REPO_ROOT)/Brewfile 2>/dev/null | cut -d'"' -f2 | tr '\n' ' ')

brew-sync:
	@if [[ "$${OSTYPE}" != "darwin"* ]]; then \
		echo "❌ brew-sync requires macOS"; \
		exit 1; \
	fi
	@echo "📦 Installing/upgrading Homebrew packages..."
	brew bundle --file=$(REPO_ROOT)/Brewfile --force
	@echo ""
	@echo "📝 Regenerating Brewfile.lock..."
	@echo "# Auto-generated by 'make brew-sync' - do not edit manually" > $(REPO_ROOT)/Brewfile.lock
	@echo "# Run 'make brew-sync' on macOS to regenerate after changing Brewfile" >> $(REPO_ROOT)/Brewfile.lock
	@brew list --versions $(BREW_PACKAGES) | sort >> $(REPO_ROOT)/Brewfile.lock
	@echo ""
	@echo "✅ Brewfile.lock updated:"
	@cat $(REPO_ROOT)/Brewfile.lock
	@echo ""
	@echo "Commit Brewfile.lock to update CI cache"

brew-check:
	@if [[ "$${OSTYPE}" != "darwin"* ]]; then \
		echo "⚠️  brew-check requires macOS (skipping)"; \
		exit 0; \
	fi
	@echo "🔍 Checking Brewfile.lock matches installed versions..."
	@ACTUAL=$$(brew list --versions $(BREW_PACKAGES) 2>/dev/null | sort); \
	EXPECTED=$$(grep -v '^#' $(REPO_ROOT)/Brewfile.lock | sort); \
	if [ "$$ACTUAL" != "$$EXPECTED" ]; then \
		echo "❌ Brewfile.lock is out of sync with installed packages"; \
		echo ""; \
		echo "Expected (from Brewfile.lock):"; \
		echo "$$EXPECTED"; \
		echo ""; \
		echo "Actual (installed):"; \
		echo "$$ACTUAL"; \
		echo ""; \
		echo "Run 'make brew-sync' to update Brewfile.lock"; \
		exit 1; \
	fi
	@echo "✅ Brewfile.lock matches installed versions"

...
# Default: shut down for snapshot creation by the host
# The host monitors for VM stop to know provisioning is complete
...
if ($NoShutdown) {
    Write-Log "Skipping shutdown as requested"
    exit $(if ($success) { 0 } else { 1 })
}

if ($NoReboot) {
    # Reboot instead of shutdown (for debugging/testing)
    Write-Log "Rebooting to apply changes..."
    Start-Sleep -Seconds 2
    Restart-Computer -Force
} else {
    # Default: shut down so the host can create a snapshot
    # This signals to the host that provisioning is complete
